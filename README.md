...
从wildcards文件中按顺序提取提示词，每次执行切换到下一个提示词，避免随机抽取的重复和遗漏问题。

#### 功能特点
- 从wildcards文件中按顺序提取提示词，每次执行自动切换到下一个
- 支持每个文件单独维护索引，不同文件之间互不干扰
- 支持重置功能，可以随时回到第一个提示词
- 自动缓存文件内容，提高性能
- 支持添加固定前缀
- 输出当前索引和文件信息，便于跟踪进度
